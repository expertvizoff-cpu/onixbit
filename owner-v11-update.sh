...
umask 022
[[ "$RELEASE_ID" =~ ^[a-z0-9-]+$ ]]
base_id=20260921-homev9-49e914a106
...
expected_proxy_re='^ec74f28ceb339084d7bf8826cd9788b13d55bfa7b8d095f47351f8485450b51f$'
...
test "$(cat "$base/.next/BUILD_ID")" = MQBQpABHTtcjG7p071XQY
...
PY
docker cp "$candidate" "$proxy:/tmp/onixbit-home-v11.caddy"
docker exec "$proxy" caddy validate --config /tmp/onixbit-home-v11.caddy --adapter caddyfile
...
echo 'Homepage footer and contact alignment correction published; root HTML, production application and proxy start time unchanged.'
