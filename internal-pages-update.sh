...
umask 022
[[ "$RELEASE_ID" =~ ^[a-z0-9-]+$ ]]
base_id=20260920-materials-c5ce94d3a9
...
expected_proxy=8d6d91322f1fbc3d10715758c26ca1f38bdc31945dfde3cbc22e0e68a8a1a241
...
test "$(cat "$base/.next/BUILD_ID")" = 8AkUrlLp-EGstF3Qiqr8i
...
PY
docker cp "$candidate" "$proxy:/tmp/onixbit-internal.caddy"
docker exec "$proxy" caddy validate --config /tmp/onixbit-internal.caddy --adapter caddyfile
...
echo 'Adapted internal pages preview published; root HTML, production application and proxy start time unchanged.'
