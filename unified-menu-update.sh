...
umask 022
[[ "$RELEASE_ID" =~ ^[a-z0-9-]+$ ]]
base_id=20260920-internal-f34f033b85
...
expected_proxy=8885944d38414f9956dc302e8b9b21395bcb7d95cab2563a4424ce118e2c7347
...
test "$(cat "$base/.next/BUILD_ID")" = paRTun1YY9l0IUoB7Lg2h
...
PY
docker cp "$candidate" "$proxy:/tmp/onixbit-unified-menu.caddy"
docker exec "$proxy" caddy validate --config /tmp/onixbit-unified-menu.caddy --adapter caddyfile
...
echo 'Unified complete navigation preview published; root HTML, production application and proxy start time unchanged.'
