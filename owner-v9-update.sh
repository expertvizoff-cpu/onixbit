...
umask 022
[[ "$RELEASE_ID" =~ ^[a-z0-9-]+$ ]]
base_id=20260921-homev7-b46d4056db
...
expected_proxy_re='^bc79be71471a9e4bbee05e2952f04e4f4988abd8e8525f7ca5b23e656786a350$'
...
test "$(cat "$base/.next/BUILD_ID")" = wEa4Jzyv6roW2dYN0c5j4
...
PY
docker cp "$candidate" "$proxy:/tmp/onixbit-home-v9.caddy"
docker exec "$proxy" caddy validate --config /tmp/onixbit-home-v9.caddy --adapter caddyfile
...
echo 'Homepage direct routes and contact glass correction published; root HTML, production application and proxy start time unchanged.'
