...
umask 022
[[ "$RELEASE_ID" =~ ^[a-z0-9-]+$ ]]
base_id=20260921-homev13-ce272c84bf
...
expected_proxy_re='^fdd4fe7cace490f6b9058f73df8a6982cdc26883b57d92e29b7c81[0-9a-f]{2}b5a8562d$'
...
test "$(cat "$base/.next/BUILD_ID")" = wU8lXI2ypOqNzMhXaBg0y
...
PY
docker cp "$candidate" "$proxy:/tmp/onixbit-founder-v14.caddy"
docker exec "$proxy" caddy validate --config /tmp/onixbit-founder-v14.caddy --adapter caddyfile
...
echo 'Homepage owner portrait replacement published; root HTML, production application and proxy start time unchanged.'
