#!/usr/bin/env bash
# /design-only update. Never switches current or the production web.
set -euo pipefail
umask 022
...
[[ "$BASE_RELEASE_ID" = 20260924-bitrix24-service-3fbbc607d0 && "$BASE_BUILD_ID" = 6lU70yKGpHopc-fTN7Hlt ]]
...
echo "[preflight] proxy fingerprint"
current_proxy_sha="$(sha256sum "$live" | cut -d' ' -f1)"
current_container_proxy_sha="$(docker exec "$proxy" sha256sum /etc/caddy/Caddyfile | cut -d' ' -f1)"
test "$current_proxy_sha" = "$current_container_proxy_sha"
printf "CURRENT_PROXY_SHA_BASE64="
printf "%s" "$current_proxy_sha" | base64 | tr -d '\n'
printf "\n"
test "$current_proxy_sha" = "$EXPECTED_PROXY_SHA"
test "$(stat -Lc '%d:%i' "$live")" = "$(docker exec "$proxy" stat -Lc '%d:%i' /etc/caddy/Caddyfile)"
echo "[preflight] package fingerprint"
test "$(sha256sum "$bundle" | cut -d' ' -f1)" = "$PACKAGE_SHA"
echo "[preflight] base release files"
...
echo "[preflight] previous preview container"
test "$(docker inspect --format '{{.State.Running}}' "$old_container")" = true
echo "[preflight] target is fresh"
test ! -e "$target"
...
echo "[preflight] production identity"
...
PY
docker cp "$candidate" "$proxy:/tmp/onixbit-cases-preview.caddy"
docker exec "$proxy" caddy validate --config /tmp/onixbit-cases-preview.caddy --adapter caddyfile
...
echo 'Reviewed /design cohesive Bitrix24 preview published. Production root HTML, application, current link and proxy start time preserved.'
