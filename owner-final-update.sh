...
umask 022
[[ "$RELEASE_ID" =~ ^[a-z0-9-]+$ ]]
base_id=20260921-homefix-2df64bc0b9
...
expected_proxy=26be0e2b4cc4f0b99245dd633cbf83df36995a20d06205b9e01b1d41eb1585c1
...
test "$(cat "$base/.next/BUILD_ID")" = IWUpbkXbfM4-pX6-NhreG
...
PY
docker cp "$candidate" "$proxy:/tmp/onixbit-owner-final.caddy"
docker exec "$proxy" caddy validate --config /tmp/onixbit-owner-final.caddy --adapter caddyfile
...
echo 'Homepage owner final correction published; root HTML, production application and proxy start time unchanged.'
