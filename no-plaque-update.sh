...
umask 022
[[ "$RELEASE_ID" =~ ^[a-z0-9-]+$ ]]
base_id=20260921-ownerfinal-e7fcb86787
...
expected_proxy_re='^028b3d68573b552d8aa1cface319e6894dbd6[0-9a-f]{2}32ff7fbb642f85ffbd142d5f0$'
expected_proxy="$(sha256sum "$live" | cut -d' ' -f1)"
[[ "$expected_proxy" =~ $expected_proxy_re ]]
...
test "$(cat "$base/.next/BUILD_ID")" = 8-Saqgo2LkBtodeOBmK35
...
PY
docker cp "$candidate" "$proxy:/tmp/onixbit-no-plaque.caddy"
docker exec "$proxy" caddy validate --config /tmp/onixbit-no-plaque.caddy --adapter caddyfile
...
echo 'Homepage light Premium backing correction published; root HTML, production application and proxy start time unchanged.'
