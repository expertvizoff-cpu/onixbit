...
umask 022
[[ "$RELEASE_ID" =~ ^[a-z0-9-]+$ ]]
base_id=20260921-noplaque-e3bdea5914
...
expected_proxy_re='^208281e1849de4e5d9fd5c86024689dce[0-9a-f]{2}49eca5dd80b70abae2ab24bb3f2cf$'
...
test "$(cat "$base/.next/BUILD_ID")" = NgAwCN8l7IAVwprw0Zg6o
...
PY
docker cp "$candidate" "$proxy:/tmp/onixbit-home-v7.caddy"
docker exec "$proxy" caddy validate --config /tmp/onixbit-home-v7.caddy --adapter caddyfile
...
echo 'Homepage owner blocks polish published; root HTML, production application and proxy start time unchanged.'
