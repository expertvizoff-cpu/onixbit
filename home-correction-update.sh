...
umask 022
[[ "$RELEASE_ID" =~ ^[a-z0-9-]+$ ]]
base_id=20260921-home-18b8a7f51a
...
expected_proxy_pattern='^08cbd9b78de558d4d6f2fd5e44bdbaf0ee34e750d1ea05e158e3[0-9a-f]{2}a6a401e5e0$'
...
test "$(cat "$base/.next/BUILD_ID")" = B_igfNwBg3Ui2V7rT-H1q
...
PY
docker cp "$candidate" "$proxy:/tmp/onixbit-home-correction.caddy"
docker exec "$proxy" caddy validate --config /tmp/onixbit-home-correction.caddy --adapter caddyfile
...
echo 'Homepage landscape correction published; root HTML, production application and proxy start time unchanged.'
