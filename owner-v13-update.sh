...
umask 022
[[ "$RELEASE_ID" =~ ^[a-z0-9-]+$ ]]
base_id=20260921-homev11-0f440c650d
...
expected_proxy_re='^5d5e01f785dc5e28153a5c1107ef1ebb15df56d06502361d7b86ef437ed0274b$'
...
test "$(cat "$base/.next/BUILD_ID")" = nsqKZwQq2yY8q4qxSenSt
...
PY
docker cp "$candidate" "$proxy:/tmp/onixbit-home-v13.caddy"
docker exec "$proxy" caddy validate --config /tmp/onixbit-home-v13.caddy --adapter caddyfile
...
echo 'Homepage compact contact composition published; root HTML, production application and proxy start time unchanged.'
