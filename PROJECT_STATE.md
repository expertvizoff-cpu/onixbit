...
- After substantial website work, default to publishing when the work is complete: run checks, commit, and push to `main` so GitHub Actions / FirstVDS deployment can start, unless Aleksander explicitly asks to keep changes local or pause before publishing.
- Do not ask Aleksander in new chats whether finished website work should be published; follow the auto-publish rule above and report the result after pushing.
...
- Current untracked local files are expected and should not be shipped unless intentionally revived: `CODEX_SETUP_RU.md`, `author-reference-photos/`, `output/`, `src/components/VideoTrustWidget.tsx`, `src/app/video-trust-widget.css`, and `tmp/`.
